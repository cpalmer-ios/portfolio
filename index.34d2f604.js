...
window.TextAnimator = class {
    ...
    animate() {
        ...
        chars.forEach((char, position)=>{
            ...
            for(let i = 0; i < 3; i++)// Use onStart and onComplete callbacks instead of trying to tween innerHTML
        ...
        if (this.textElement) masterTimeline.to(this.textElement, 0.5, {
            '--anim': 1,
            ease: Power2.easeOut
        }, 0);
    }
    ...
    reset() {
        ...
        try {
            ...
            const chars = this.splitter.getChars();
            if (chars && chars.length > 0) chars.forEach((char, index)=>{
                if (char) {
                    TweenLite.killTweensOf(char);
                    // Directly set innerHTML instead of using TweenLite
                    if (index < this.originalChars.length) char.innerHTML = this.originalChars[index] || '';
                }
            });
            else // If no chars are found, restore from original text
            if (this.originalText) this.textElement.textContent = this.originalText;
...
//# sourceMappingURL=index.34d2f604.js.map
