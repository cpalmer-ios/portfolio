...
(function () {
    // Wait for both GSAP and TextAnimator to be available
    function initializeAnimations() {
        if (!window.gsap || !window.TextAnimator) {
        ...
        document.querySelectorAll('.list__item').forEach(item => {
            ...
            const animators = cols.map(col => new window.TextAnimator(col));
        ...
        document.querySelectorAll('a.hover-effect').forEach(item => {
            const animator = new window.TextAnimator(item);
            item.addEventListener('mouseenter', () => {
                animator.animate();
            });
            item.addEventListener('mouseleave', () => {
